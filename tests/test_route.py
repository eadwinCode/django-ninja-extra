...
@router("permission", permissions=[permissions.IsAuthenticated & permissions.IsAdminUser])
...
class TestControllerRoutes:
    ...
    def test_controller_route_build_accurate_operations_list(self, path, operation_count):
    ...
    def test_controller_route_should_have_an_operation(self):
        for route_func in SomeTestController.get_route_functions():
            ...
            operations = list(filter(
                lambda n: n.operation_id == route_func.route_definition.route_params.operation_id,
                path_view.operations
            ))
            assert len(operations) == 1
            assert route_func.route_definition.route_params.methods == operations[0].methods
...
class TestRouteFunction:
    ...
    def test_get_required_api_func_signature_return_filtered_signature(self):
        route_function = RouteFunction(route_definition=route.get(''), api_func=self.api_func)
        ...
        route_function = RouteFunction(route_definition=route.get(''), api_func=self.api_func_with_has_request_param)
...
class TestAPIControllerRoutePermission:
    ...
    def test_route_is_protected_by_global_controller_permission(self):
        ...
        assert 'You do not have permission to perform this action.' in str(pex.value.message)
